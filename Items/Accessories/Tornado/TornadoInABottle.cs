        ...
        public override void UpdateAccessory2(Player player, bool hideVisual)
        {
            ...
            if (player.grappling[0] == -1 && !player.tongued && (player.jump == 0 || player.frozen)
                ...
                || (!(player.itemAnimation > 0 && player.inventory[player.selectedItem].useStyle != 10)
                    && !player.mount.Active
                    &&
                    !(player.inventory[player.selectedItem].holdStyle == 1 &&
                      (!player.wet || !player.inventory[player.selectedItem].noWet))
                    &&
                    !(player.inventory[player.selectedItem].holdStyle == 2 &&
                      (!player.wet || !player.inventory[player.selectedItem].noWet))
            ...
            if (player.controlJump && (player.releaseJump || (player.autoJump && onGround)))
            {
                ...
                {
                    if (player.releaseJump) jumpAgain = true;
                }
            ...
            if (dJumpEffect)
            {
                     ...
                     (player.gravDir <= -1f && player.velocity.Y > 0f)))
                {
                    ...
                    for (var i = 0; i < tornadoWidth; i++)
                    {
                        ...
                        var gore =
                            Gore.NewGore(
                                new Vector2(player.Center.X + xOff - 18, player.position.Y + (float) (playerHeight/2)),
                                new Vector2(-player.velocity.X, -player.velocity.Y), Main.rand.Next(61, 64)
                                /*Main.rand.Next(220, 223)*/, scale);
                        //Main.gore[gore].color = grayMultiplier;
                        Main.gore[gore].velocity = player.velocity*0.3f*scale;
                        Main.gore[gore].velocity.X -= xOff/(player.width*tornadoWidth)*
                                                      (1 + Main.rand.NextFloat()*tornadoWidth);
                        ...
                        if (player.inventory[player.selectedItem].holdStyle == 2)
                        {
                            if (player.inventory[player.selectedItem].type == 946)
                            {
                                player.itemLocation.X = player.position.X + (float) player.width*0.5f -
                                                        (float) (16*player.direction);
                            }
                            if (player.inventory[player.selectedItem].type == 186)
                            {
                                player.itemLocation.X = player.position.X + (float) player.width*0.5f +
                                                        (float) (6*player.direction);
